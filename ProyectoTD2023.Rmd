...
ggplot(df_digital) +
  ...
  theme_minimal()
```
Como se puede observar, sí, hay diferencias notables en el ritmo cardíaco fetal entre el inicio y el final del parto. Al principio es común que el ritmo cardíaco fetal fluctue ligeramente.
...
Como se puede observar en la graficas, ambas presiones empiezan desde cero y se disparan al mismo tiempo hasta valores de casi 100 para la presion diastolica y superiores a 150 para la sistolica. Una vez tienen esta subida, se mantienen en susu valores hasta casi el final del parto donde ambas tienen una pequeña bajada y nuevamente continuan siendo constantes.
...
ggplot(df_digital) +
  ...
  theme_minimal()
```
En parametros como el TOCO se puede ver que funcionan siguiendo una serie de patrones. En otros parametros como el ritmo cardiaco son mas constantes aunque teniendo pequeños cambios. Y en los parametros que nos indican las presiones podemos observar que solamente tienen dos cambios, al principio del embarazo y al final.
